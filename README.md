...
> 💡 **ПРИМЕЧАНИЕ:** Ключ создаётся **один раз**.<br>После создания, при каждом использовании `Lock Monitor.exe`, создавать ключ повторно **не требуется**!
...
требуется указать имя машины в параметре `sourceSRV`, где хранится папка со скриптом.
 ...
 👍 **РЕКОМЕНДАЦИЯ:**
Указать название изображения PNG в параметре `namePNG`, например `picture.png`
...
⚠️ **ВАЖНО:**
Требуется самостоятельно собрать скрипт `Lock-Monitor.ps1` и `Lock-Pin.ps1` в `.exe` через программу **Win-PS2EXE**.
